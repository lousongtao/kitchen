...
//                        startProgressDialog("", "start posting data ... ");
    ...
    private void changeSoldoutStatus(){
        ...
        if (response.get() == null) {
            Log.e("DishSoldListener", "Error occur while change dish "+ dish.getFirstLanguageName() +" soldout status. response.get() is null.");
            MainActivity.LOG.error("Error occur while change dish "+ dish.getFirstLanguageName() +"soldout status. response.get() is null.");
            progressDlgHandler.sendMessage(CommonTool.buildMessage(PROGRESSDLGHANDLER_MSGWHAT_SHOWEXCEPTION, "Error occur while make dish "+ dish.getFirstLanguageName() +" soldout = " + String.valueOf(!dish.isSoldOut())+ ". response.get() is null."));
            return;
        }
